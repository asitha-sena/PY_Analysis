...
def dyn_lat_tower_1(L=10., D=1., A=1., I=1., t=0.1, E=200e9, rho=7850., n=20, modes=[1,2], 
    ...
    for i in modes:
        ...
        plot(u/u[0],node_loc), xlabel('Displacement'), ylabel('Height')
        grid(True)
...
def dyn_lat_tower_4(soil_profile, D_tower_bot=4.5, D_tower_top=4.5, t_tower_bot=0.027, t_tower_top=0.027, 
    ...
    k_secant = calc_k_secant(soil_profile, node_loc, n_elem, n_pile, n_tower, D_pile, L_pile, L_tower, py_model=py_model,
                             Y_secant=Y_secant, plot_profile=plot_profile)
    
    #Convert units
    k_secant = 6923.*k_secant
...
def calc_k_secant(soil_profile, node_loc, n_elem, n_pile, n_tower, D_pile, L_pile, L_tower, py_model='Matlock',
    ...
    *** Note: The soil profile data is provided in US customary units which pile geometry is specified in SI units. ***
    ...
    for i in range(0,n_elem):
        ...
        if py_model=='Matlock':
            k_secant[i] = matlock_k_secant(z[i], D_pile*39.4, Su, sigma_v_eff, Y_secant=Y_secant, z_0=z_0, epsilon_50=0.02)
        elif py_model=='Jeanjean':
            k_secant[i] = jeanjean_k_secant(z[i], D_pile*39.4, Su, sigma_v_eff, Y_secant=Y_secant, z_0=z_0, Su_0=f_Su(z_0), A=550)
        elif py_model=='Modified Matlock':
            k_secant[i] = modified_matlock_k_secant(z[i], D_pile*39.4, Su, sigma_v_eff, Y_secant=Y_secant, z_0=z_0, epsilon_50=0.02)
        elif py_model=='Kodikara':
            k_secant[i] = kodikara_k_secant(z[i], D_pile*39.4, Su, sigma_v_eff, Y_secant=Y_secant, z_0=z_0, R1=0.5, A=250, print_curves='No')
        else:
    ...
    if plot_profile=='Yes':
        ...
        twinx()
        plot([0,0],[0,1.2*(L_tower+L_pile)],'.')
        ylim([0,1.2*(L_tower+L_pile)])
        ax2 = gca()
        ax2.invert_yaxis()
        ylabel('(m)')
